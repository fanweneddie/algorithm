...
void search_node_special(RBTree* tree,RBNode* node,
            int key_search,int* results,int * results_len);
            ...
            int key_search,int* results,int * results_len)
{
    ...
    {
        results[*results_len] = key_search;
    ...
    else
    {
        if(key_search < node->key)
        {
            // possibly found, keep on searching
            if(node->left != tree->nil)
            {
                search_node_special(tree,node->left,
                    key_search,results,results_len);
                return;
            }
            // can't find,search for predecessor instead
            else
            {
                results[*results_len] = RB_predecessor(tree,node)->key;
                *results_len = *results_len + 1;
                return;
            }
        }
        ...
        else
        {
            ...
            {
                search_node_special(tree,node->right,
                        key_search,results,results_len);
                return;
            }
            else
            {
                results[*results_len] = node->key;
                *results_len = *results_len + 1;
                return;
            }
        }// else
...
int main()
{
    ...
    char cmd[10];
    // the value to insert into database
    int insert_value;
    // the value to search in FIND command
    int search_value;
    // the RBTree to store the keys and values in database
    RBTree* data_base = init_RBTree();
    // the array to store the output result
    int results[2000000];
    // the length of array results
    int* results_len = (int*)malloc(sizeof(int));
    //-----------------------------------------------
    // stage of insertion
    scanf("%s",cmd);
    ...
    {
        // INSERT command
        while(strcmp(cmd,"INSERT") == 0)
        {
            // insert the value into database
            scanf("%d",&insert_value);
            RBNode* new_node = init_RBNode(NULL,insert_value);
            RB_insert(data_base,new_node);
            scanf("%s",cmd);
        }
        //-----------------------------------------------
        // stage of searching
        while(strcmp(cmd,"FIND") == 0)
        {
            // input the value to search
            scanf("%d",&search_value);
            // search for that value in RBTree
            search_node_special(data_base,data_base->root,
                    search_value,results,results_len);
            // store the result
            scanf("%s",cmd);
        }
    }
